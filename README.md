...
Research Context • Purpose and Scope • Main Functions • Architecture • Quick Start • License
</div>
---
Research Context
...
---
Workflow
Upload operational data: Provide a compatible vehicle circulation plan or use the included non-confidential example input.
Review preprocessing: Inspect the derived vehicle availability and trip-energy structures.
Configure the depot: Define vehicle assumptions, charger capacity, charging sockets, grid connection limits, and state-of-charge requirements.
Configure prices: Select time-varying or fixed electricity prices and define additional variable procurement components.
Select a scenario: Run the optimized baseline, PV, BESS, or combined PV-BESS model.
Execute the optimization: Solve the configured model and review solver and feasibility information.
Analyze the results: Inspect charging schedules, state-of-charge profiles, costs, grid imports, PV utilization, and BESS operation.
Compare and export: Compare scenarios and export relevant result data.
---
...
---
License
The repository is provided for non-commercial academic, research, educational, and evaluation purposes under the terms stated in the LICENSE file.
